...
- **(v12.1)** Если передан `delivery_address` (строка) — сайт ищет существующий адрес пользователя по точному совпадению текста. Если не найден — создаёт новую запись в `delivery_addresses` с `name = "Адрес из 1С"`
...
- [ ] `delivery_address` сохраняется в `delivery_addresses` и привязывается к заказу (v12.1)
