...
- **(v15)** Поле `manager` (опциональное) — задекларировано для совместимости на будущее; 1С пока не включает его в `partner.updated`, но при наличии — обрабатывается аналогично `partner.created`
...
- [ ] **(v15)** При `manager` в payload — обработка аналогична `partner.created`
