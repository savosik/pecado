...
- Required поля: `event`, `message_id`, `uuid`. `partner_uuid` требуется для
  fallback-матчинга и для перепривязки владельца.
...
### Перепривязка контрагента к другому партнёру (v15.14.0)

В 1С контрагента можно перевести на другого партнёра. Сайт обязан повторить это
перемещение, иначе клиент не увидит свою организацию в личном кабинете и
не сможет оформить заказ — блок «Организация» окажется пустым.

| Ситуация | Что делает сайт |
|---|---|
| `partner_uuid` не передан | `user_id` не трогает |
| `partner_uuid` передан, партнёр по `User.erp_id` **не найден** | warning, `user_id` **не трогает** — владельца не обнуляем, иначе опечатка в 1С осиротит контрагента |
| Партнёр найден, совпадает с текущим владельцем | ничего не меняет |
| Партнёр найден и **отличается** от текущего владельца | переносит `companies.user_id` на нового пользователя, пишет в лог `user_id_from` / `user_id_to` |
| У Company `user_id = NULL` (контрагент пришёл раньше партнёра) | привязывает к найденному партнёру |

- Переносится **только сама организация**. Ранее созданные заказы, отгрузки, платежи
  и балансы остаются за прежним пользователем — сайт историю не переписывает.
  Если требуется перенести и историю, это отдельная ручная операция.
- Смена `user_id` **не** триггерит обратную публикацию `contractor.updated` в 1С:
  `user_id` не входит в список значимых полей `PublishContractorToErp`, а обновление
  идёт через `Company::withoutEvents()`.
- Банковские счета привязаны к Company, а не к пользователю, и переезжают вместе с ней.

!!! warning "До v15.14.0 перепривязка молча игнорировалась"
    Обработчик обновлял только реквизиты и не трогал `user_id`: сообщение
    отрабатывало со статусом `success`, но контрагент оставался у старого партнёра.
    Симптом на сайте — у клиента 0 организаций и неактивная кнопка «Оформить заказ».
...
- [ ] `contractor.updated` с изменившимся `partner_uuid` переносит Company на нового пользователя (v15.14.0)
- [ ] `contractor.updated` с неизвестным `partner_uuid` не обнуляет и не меняет текущего владельца Company
