...
## Промо-позиции: полный круг (v15.6)

**Зависимости:** 2.6

- [ ] Оформление с акцией даёт на сайте и в 1С одинаковый набор документов
- [ ] `order.updated` из 1С по промо-заказу **не затирает** привязку позиции к правилу
      акции (`promotion_rule_id`) и вид позиции (`promo_kind`)
- [ ] Позиция, добавленная менеджером **вручную в 1С**, приезжает на сайт, сохраняется
      и переживает следующий `order.updated`
- [ ] Повторная доставка того же `order.created` (тот же `message_id`) не задваивает
      позиции промо-заказа
- [ ] `shipment.created` по промо-заказу закрывает его так же, как обычный
