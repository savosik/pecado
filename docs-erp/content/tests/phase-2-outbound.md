...
**Зависимости:** 2.6, склад «Москва реклама» с заполненным `external_id`

🟢 Акция с наградой вида «рекламный образец» → оформить заказ.

- [ ] `type: "promo_sample"`, `warehouse_uuids` содержит склад «Москва реклама»
- [ ] Позиции несут `promo_kind: "sample"`
- [ ] Позиции **не выписываются** в накладную клиенту
- [ ] Если у склада «Москва реклама» **не заполнен** `external_id` — заказ **не публикуется**,
