...
Сайт больше не присылает **нулевую базовую цену** в позиции обычного заказа. Структура
сообщений не меняется — правка приводит сайт к уже действующему правилу контракта.

### Предыстория

23.09.2026 заказы 29УТ-014790 и 29УТ-014791 («Отель Подушкин») не провелись: строка с
артикулом `955067-L` встала в документе с ценой и суммой 0. Сайт прислал по ней
`base_price: 0`, `discount_percent: 0`, `final_price: 1212`. Для обычного заказа контракт
задаёт тождество `final_price = base_price × (1 − discount_percent / 100)` (авторитетность
`final_price` объявлена только для `promo`, `promo_sample` и `defect`), поэтому 1С
посчитала строку от нулевой базы — по спецификации.

Ноль в базе появился так: карточка `955067-L` заведена сообщением `product.created`
02.09.2026, а базовая цена на сайте меняется **только** событием `price.updated` — по
этому UUID оно не приходило ни разу. Продавался товар по индивидуальной цене из выгрузки
(1212 ₽), поэтому на витрине выглядел нормально.

### Изменено (контракт)

- `order.created`, `order.updated` (Сайт → 1С): у `items[].base_price` зафиксирована
  гарантия сайта — в позициях `type: "order"` и `type: "preorder"` значение больше нуля
  и не меньше `final_price`. Если у карточки нет прайсовой цены, сайт присылает
  `base_price = final_price` и `discount_percent = 0` — так же, как это давно сделано
  для уценки (v15.9.3).
- Описание `final_price` в JSON Schema приведено к AsyncAPI: «берётся авторитетно» —
  про `promo`, `promo_sample` и `defect`, а не про любой заказ.
- Правила цен: зафиксировано, что базовая цена карточки на сайте живёт только из
  `price.updated`, и добавлен критерий приёмки — событие должно приходить хотя бы раз
  по каждому товару с назначенной ценой.

### Изменено (сайт)

- Сборщик заказов схлопывает базовую цену в цену клиента, когда базовая нулевая или
  ниже клиентской (наценка): раньше наценка тоже уезжала неконсистентной парой.
- Позиция, у которой нет ни базовой, ни индивидуальной цены, **не оформляется**: витрина
  отвечает клиенту по-русски, клиентское API возвращает её в `not_accepted` с причиной
  `no_price`. Промо-позиции с нулевой ценой (подарки, пробники) работают как прежде.
...
**Совместная отгрузка резервов.** Клиент-интернет-магазин отмечает несколько заказов
в резерве и отправляет их в отгрузку вместе; 1С оформляет по группе минимальный комплект
реализаций и расходных ордеров (одна реализация и один ордер, если позволяют владельцы
товара, налоговый режим и складские ключи). Объединяется **отгрузка, а не заказы**: заказы
остаются отдельными документами. Согласовано в топике №7 Agent Hub (resolution 18.09.2026).
**Контракт аддитивен**: без новых полей поведение прежнее.

### Добавлено (контракт)

- `order.confirmed` (Сайт → 1С): необязательная пара полей `ship_together_key` (UUID группы)
  и `ship_together_order_uuids` (манифест — UUID всех заказов группы, минимум 2, одинаковый
  во всех сообщениях группы). Присутствуют только вместе.
- `order.updated` (1С → Сайт): `ship_together_key`, `ship_together_status`
  (`confirmed` | `conflict`), `ship_together_conflict {reason, message, order_uuid}` с кодами
  `timeout`, `manifest_mismatch`, `not_reserved`, `incompatible`, `merge_forbidden`,
  `shortage`, `processing_error`.

### Правила

- Сайт до публикации проверяет: один партнёр, контрагент, валюта, **ровно один склад**,
  способ и адрес доставки, все заказы — действующие резервы; сообщения группы уходят одной
  транзакцией. Остальное (организация, соглашение, договор, запреты объединения, остатки)
  проверяет 1С под групповой блокировкой.
- 1С копит группу по манифесту (`collecting → confirmed | conflict`), тайм-аут 15 минут
  от сохранения первого валидного сообщения группы; полная группа обрабатывается сразу
  одним групповым вызовом движков, поштучный автоордер подавлен.
- Отказ — по всей группе, частичных `goods_issue.*` / `shipment.*` нет; итог на ключ один
  и окончательный; повтор после отказа — новый ключ и новые `message_id`.
- Сайт не снимает резерв локально до итога из 1С, правки и отмена заказов группы закрыты
  на время ожидания.
- Включение — по отдельному решению владельцев после совместных испытаний (тест-план Р-7).

## [16.10.0] — 2026-09-10

Персональный менеджер партнёра **больше не принимается из 1С**. Закрепление менеджера
за партнёром (и решение не закреплять никого — лид) ведёт руководитель отдела продаж
в CRM сайта. **Контракт обратно совместим**: поле `manager` остаётся в схемах, но
помечено `deprecated` и на данные сайта не влияет.

### Предыстория

- Поле `manager` в `partner.created` / `partner.updated` появилось в v15 / v15.1, но 1С
  ключ фактически не присылает: распределение клиентов по менеджерам с августа 2026
  ведётся на сайте вручную, а «ничейные» партнёры вынужденно числились за карточкой РОПа,
  потому что партнёр без менеджера в CRM был невидим.
- Если бы 1С начала слать `manager`, ручное распределение было бы затёрто.

### Изменено (контракт)

- `partner.created`, `partner.updated`: поле `manager` — `deprecated`, сайт его игнорирует
  (объект и `null` равнозначны отсутствию ключа). Вложенные `uuid`/`name` больше не
  обязательны — некорректный объект не валит сообщение. 1С может перестать слать поле
  в любой момент без согласования.
- Справочник менеджеров сайта (`personal_managers`) из шины больше не пополняется.

### Изменено (сайт)

- CRM: в карточке партнёра руководитель отдела (право `crm-clients-all.edit`) закрепляет
  менеджера или снимает закрепление; смена пишется в журнал статусов партнёра.
- CRM: партнёры без менеджера показываются по личной галочке сотрудника
  «Нераспределённые» (рядом с «Только мои», во всех разделах разом); в списке партнёров
  фильтр «Без менеджера».

## [16.9.2] — 2026-09-09

Флаг `reserve` на `order.created` сделан **авторитетным признаком обеспечения**
(топик №6 Agent Hub, согласовано 09.09.2026). **Контракт обратно совместим**:
поле остаётся optional, но `absent = false` зафиксировано явно.

### Предыстория (дефект)

- Сайт слал `reserve` **только при `true`**, при выборе «к отгрузке» (`false`) поле
  опускал. 1С трактовала отсутствие/`false` одинаково: процедура резерва выходила,
  но заказу **до неё** уже проставлялся статус `КОбеспечению` и вариант строк
  `СоСклада`. Итог: заказ клиента, выбравшего «к отгрузке», фактически резервировался
  вопреки выбору. Симметричный дефект на обеих сторонах.

### Изменено (контракт)

- `reserve` на `order.created` — **авторитетный признак**, шлётся явным Boolean во
  **всех** новых `order.created` (не только по участникам режима):
  - `reserve: true` (режим вкл ∧ участник `reserve_allowed` ∧ срок валиден) → статус
    `КОбеспечению`, строки `СоСклада`, `Доп_ЗаказВРезерве = true`, срок из `reserved_until`;
  - `reserve: false` → статус `КОтгрузке`, строки `Отгрузить`, **без резерва**;
  - поле отсутствует / `null` → совместимый fallback как `false`, резерв по умолчанию
    **НЕ ставится**.
- Право создать резерв всё равно проверяет 1С (режим ∧ участник ∧ срок); по
  неучастнику `reserve: true` игнорируется и возвращается `reserve: false` ответным
  `order.updated`.

### Исправлено (сайт)

- Исходящий `order.created` (`PublishOrderToErp`) отправляет `reserve` (Boolean) в
  каждом сообщении; `reserved_until` — при `reserve: true`.

### На стороне 1С (по договорённости)

- Узкая правка default-path: обычный заказ сначала готовится как `КОтгрузке` со
  строками `Отгрузить`, и только валидный `reserve: true` переводит его в резерв.
  Предзаказы и аварийные `defect` / `promo` сохраняют текущее поведение.

## [16.9.1] — 2026-09-05

Уточнение семантики `line_number` по вопросу 1С из топика №6 раскатки резервов.
**Контракт не ломается**, меняется формулировка и усиливается проверка на сайте.

### Изменено

- `line_number` описан как стабильный **в пределах версии состава**, а не на всём
  жизненном цикле документа: при физическом удалении строки (сценарий Р-2 —
  клиент убирает позицию из резерва) платформа 1С перенумеровывает последующие.
  Ключ строки — **пара `items_version` + `line_number`**; состав всегда приходит
  и применяется целиком (snapshot), сопоставление идёт внутри сообщения.
- Требование к 1С зафиксировано явно: **`items_version` растёт при любом
  изменении состава**, включая перенумерацию.

### Исправлено (сайт)

- Клиентский API правки состава требует `base_items_version` (было: сайт
  подставлял текущую версию сам) и отклоняет устаревшую правку кодом
  `stale_items_version` со статусом 409 — **до** публикации в шину. Прежнее
  поведение допускало гонку: интеграция магазина со старым представлением
  могла уменьшить не ту строку, потому что идентификатор строки стабилен
  только внутри версии. Проверка 1С остаётся вторым рубежом.
- Кабинет отправляет версию состава со страницы заказа и получает тот же отказ
  с предложением обновить данные.

### Уточнено

- **`status` в `order.updated` опционален** — отсутствие ключа и `null` означают
  «статус не меняется». Поведение было таким с v11, но не описано явно; теперь
  зафиксировано, потому что на нём держится сообщение о **распроведённом**
  документе: у него регистр состояний пуст, и 1С шлёт полный `order.updated`
  с `reserve: false` без `status` (ручное снятие резерва менеджером, а также
  `conflict`-snapshot по распроведённому заказу). После обратного проведения
  статус приезжает обычной публикацией.

## [16.9.0] — 2026-09-03

Режим **«Заказы в резерве»** — самообслуживание клиентов-интернет-магазинов.
Протокол согласован агентами сторон в топике №5 Agent Hub (resolution 03.09.2026).
Все изменения аддитивны. **Раскатка за флагами обеих сторон** — события не
отправляются до отдельного решения владельцев; до включения 1С может ничего
не менять.

### Добавлено

- **Сайт → 1С, очередь `erp_out.orders`** (раньше несла только `order.created`):
    - `order.created` + `reserve` (bool) и `reserved_until` (обязателен при
      `reserve: true`) — создать заказ со строками «Резервировать на складе»;
    - `order.updated` (новое) — правка состава клиентом: состав целиком,
      `base_items_version` для оптимистичной блокировки; в v1 только уменьшение;
    - `order.deleted` (новое) — отмена клиентом или снятие по таймауту
      (`reason`: `client_cancelled` | `reserve_expired`); в 1С — отмена строк +
      «Закрыт», **без пометки удаления**;
    - `order.confirmed` (новое) — отправить резерв в отгрузку: строки
      `СоСклада` → `Отгрузить`, статус «К отгрузке».
- **1С → Сайт**, аддитивные поля:
    - `order.created` / `order.updated`: `reserve`, `reserved_until`
      (**фактический** срок удержания — 1С урезает запрошенный до своего предела,
      настройка, стартово 24 часа), `items_version` (растёт только при изменении
      строк; `revision` остаётся счётчиком сообщений), `conflict
      {rejected_message_id, reason}` — отказ в правке при отставшей версии состава;
    - `partner.created` / `partner.updated`: `reserve_allowed` — участник режима,
      **мастер флага — 1С**; сайт показывает выбор резерва только участникам.
- Правила: [Заказы в резерве](rules/order-reserves.md) — жизненный цикл, предел
  удержания, рассинхрон флагов, гейт сборки, страховочный регламент (+6 ч грации).
- Тест-план: [Заказы в резерве](tests/reserves.md) — критерии приёмки Р-1…Р-6.

### Изменено

- `sendOrderCreated`: правило «после отправки пользователь изменять заказ не может»
  уточнено — для заказа в окне резерва клиент правит/отменяет/подтверждает сам;
  для обычных заказов правило прежнее.

## [16.8.0] — 2026-08-25

Завершение перехода на регистр взаиморасчётов (эпик fin-00, волна 4). **Контракт шины
не меняется:** события те же, схемы те же. Меняется то, что сайт с ними делает — он
перестаёт содержать вторую счётную машину.

### Удалено (на стороне сайта)

- **Второе счётное ядро.** Флаг `SETTLEMENTS_LEDGER_ENABLED` и расчёт по графику
  оплаты плюс расшифровкам платежей сняты: долг, просрочка, план поступлений, балансы,
  акт сверки и календарь считаются лентой регистра всегда.
- **Разнесение платежей** (`payment_allocations`, `payments.allocated_amount` /
  `unallocated_amount`) и **график в собственной таблице** (`shipment_payment_schedules`),
  **построчная просрочка балансов** (`contractor_balance_overdue_details`).
- **Агентские операции** `payment.balances`, `payment.unpaid-shipments`, `payment.schedule` —
  отвечают `settlement.balance` и `settlement.schedule`.

### Что это значит для 1С

Ничего не нужно менять. Поля, снятые с контракта в v16.0.0 (`allocations[]` в платежах,
`payment_schedule[]` внутри реализаций, `overdue_details[]` в балансах), сайт принимает
и **игнорирует** — `additionalProperties: true`, DLQ не будет. Присылать их незачем,
но и отключение не срочное.

Актуальные каналы про деньги: `payment_schedule.updated` (план и закрытая часть),
`settlement.posted` / `settlement.reverted` (движения), `balance.updated` (контрольная
сумма для сверки), `payment.*` (журнал поступлений с банковскими реквизитами).

### Исправлено

- **Пустой график в карточке реализации.** Блок читал таблицу, которую 1С не наполняет
  с 12.08.2026 (график переехал в `payment_schedule.updated`): у всех отгрузок после этой
  даты карточка в CRM, админке, кабинете и внешнем API показывала «графика нет».
- **Выгрузка раздела «Финансы» падала с 500** на боевом: фильтр периода обращался
  к колонке старого ядра.
- **Финансовые письма** (`mail:finance-scan`) считали сроки и просрочку по тем же
  мёртвым таблицам.

### Данные

Колонки `shipments.paid_amount / payment_status / payment_due_date` и
`orders.prepaid_amount` остаются — их читают кабинет, фильтры и внешний клиентский API, —
но пишет их теперь проекция плановых строк регистра (`SettlementProjector`).
Разовый пересчёт выполняется миграцией, дальше — командой `settlements:project-documents`.

---

## [16.7.0] — 2026-08-24

Итог круга 12 (Agent Hub, топик №3): фантомная просрочка — «Просрочено 1,14 млн ₽»
у клиента с авансовым сальдо.

### Добавлено

- **`document_kind: "initial"` в `payment_schedule.updated` — «Первичный документ».**
  Долги, заведённые переносом остатков между базами, — реальная дебиторка внешних
  клиентов (132 тыс ₽ по восьми контрагентам на момент круга), которой не было на витрине.
  Ведёт себя как реализация: построчный `settled_amount` из регистра по срокам, участвует
  в просрочке. Шлётся **только по внешним контрагентам** — расчёты между своими юрлицами
  (~6 млн ₽) в обмен не входят.

### Изменено

- **График заказа — план платежа, а не долг; просрочкой не считается.** Регистр
  `РасчетыСКлиентамиПоСрокам` заказы не ведёт: погашение по ним физически не публикуется,
  и просроченная строка заказа оставалась «просроченной» навсегда. Сайт исключает строки
  `document_kind: order` из просрочки на всех поверхностях (CRM и кабинет клиента);
  в плане поступлений и календаре они остаются как ожидаемые платежи.

- **Исполненный план заказа снимается с витрины.** При полной отгрузке или закрытии
  заказа 1С публикует ревизию графика с пустым `lines: []` (механика замены целиком
  существовала с v16.0.0 — новой обязанностью стала публикация). Критерий полноты —
  нулевой остаток регистра `ТоварыКОтгрузке` при наличии проведённой реализации,
  а не статус заказа. Частичная отгрузка график не трогает.
...
- **Excel в печатных формах: акт сверки приезжает в XLSX.** `mime_type` перестал быть
  PDF-only — принимаются `application/pdf`,
  `application/vnd.openxmlformats-officedocument.spreadsheetml.sheet` (XLSX) и
  `application/vnd.ms-excel` (XLS). Формат определяется по сигнатуре содержимого
  (`%PDF-`, `PK`, `D0 CF 11 E0`), а не по `mime_type` и не по расширению: и то и другое
  задаёт 1С. При расхождении сайт верит содержимому и пишет предупреждение в лог —
  документ из-за забытой строки в коде 1С не теряется.

  `enum` для `mime_type` намеренно не введён: неизвестный тип не должен ронять сообщение
  в очередь недоставленных, он отсекается на переносе файла статусом `rejected`.

- **`period_from` / `period_to` — период, за который построена форма.** Оба поля
  опциональны и заполняются у форм за период, прежде всего у акта сверки; `period_to`
  совпадает с `date`. Раньше период был виден только внутри файла, и в кабинете
  по нему нельзя было ни фильтровать, ни понять, чем два акта различаются.

### Изменено

- **Регламент акта сверки: ежесуточно вместо ежемесячного.** Прогон около 05:00 МСК
  строит акт за «с начала текущего года по вчера» по контрагентам с активными
  взаиморасчётами (оборот ≠ 0 **или** остаток ≠ 0), без порога отсечения по сумме;
  договоры сводятся в один акт. Ежемесячный регламент (5 числа за прошлый месяц,
  решение от 14.08.2026) отменяется — параллельно они дали бы клиенту два акта
  с разными периодами и разными долгами на одну дату.

- **Разрез акта сверки получил валюту и год.** Ключ стабильного `uuid` —
  организация × контрагент × валюта × год. Год в разрезе — не формальность:
  без него суточный прогон 1 января переписал бы период у прошлогодней формы,
  и акт за прошлый год исчез бы из кабинета, потому что сайт хранит одну актуальную
  версию и перезаписывает файл по тому же `uuid`.

  Уже отправленные PDF-акты (158 форм с 14.08.2026) попадают в тот же разрез:
  их файл заменится на XLSX по тому же `uuid`, вторая копия не появится.
  Осиротевший `.pdf` в хранилище сайт удаляет сам при смене расширения.

Согласовано со стороной 1С в топике Agent Hub «Акты сверки в Excel в кабинет клиента
раз в сутки».
...
- **`replaces_order_uuid` в `order.created` (Сайт → 1С)** — поле убрано из контракта
  вместе с механикой подборок замен. Сайт больше не предлагает клиенту замену
  отменённой позиции, значит и заказов-замен не возникает.

  **На стороне 1С делать ничего не нужно**: поле было опциональным, отправка держалась
  выключенной флагом и в боевой обмен ни разу не уходила. Payload обычных заказов
  не менялся и не меняется.

### Изменено

- **Недобор: причина отмены остаётся за человеком.** Правила заказов дополнены
  разделом «Причину отмены протокол не передаёт»: строку снимает и склад при сборке,
  и менеджер по просьбе клиента, а `order.updated` в обоих случаях одинаков.
  Сайт ведёт журнал отмен (раздел CRM «Недоборы») и просит менеджера разметить
  источник вручную, подсказывая наличием расходного ордера по заказу.

  Если в 1С есть признак причины отмены строки — сообщите: это лучший кандидат
  на расширение `items[]`, он снимет ручную разметку целиком.
