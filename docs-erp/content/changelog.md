...
- **`organization_uuid` — опциональная подсказка организации в `order.created`**
  (Сайт → 1С). Склад на сайте теперь может быть привязан к нашей организации
  (`warehouses.organization_id`, справочно, заводится в админке — «ООО Пекадо —
  Москва основной», «ИП Елисеев — Москва персональный»). Для заказов, где
  организация определена однозначно (обычно — организация склада,
  зафиксированного в регионе со стопкой складов), сайт может передать её UUID.

  Это **подсказка, не решение**: 1С по-прежнему сама выбирает и возвращает
  организацию документа (`organization` во входящем `order.updated`, правило
  v15.7.1/v15.8.0 не меняется).

  **Переходный флаг** `ERP_ORGANIZATION_UUID_ENABLED` (по умолчанию **выключен**,
  независим от `stack_warehouse_pinning`): пока выключен, поля в payload нет
  вовсе — легаси-приёмник 1С не видит нового ключа. Включать только после
  подтверждения стороной 1С. Подробности —
  [Организации → Сайт может подсказать организацию](rules/organizations.md).

## [16.5.0] — 2026-08-19

### Изменено

- **Режим стопки складов: фиксация склада сайтом в `order.created`** (Сайт → 1С).
  У региона на сайте может быть включена приоритетная последовательность складов
  («стопка»): по каждому товару действуют остаток и цена верхнего склада с наличием,
  нижние — фолбэк. Для таких регионов:

    - группа `order` одного оформления разбивается по складам стопки — документов
      может быть **больше пяти**;
    - `warehouse_uuids` каждого документа содержит **ровно один UUID** —
      зафиксированный сайтом склад; 1С проводит строго по нему, фолбэк на
      «ОсновнойСклад» не применяется;
    - `preorder` / `defect` / `promo` / `promo_sample` не затронуты.

  **Схема не меняется** (`warehouse_uuids` остаётся массивом) — меняется только
  семантика. Для регионов без стопки поведение прежнее: склады взаимозаменяемы,
  конкретный выбирает 1С.

  **Легаси-совместимость**: фиксация склада в исходящих сообщениях закрыта
  переходным флагом `ERP_STACK_WAREHOUSE_PINNING_ENABLED` (по умолчанию
  выключен, паттерн `replaces_order_uuid` v16.2.0). Пока он выключен, даже
  заказы регионов со стопкой уходят прежним перечислением складов региона —
  легаси-приёмник 1С не требует никаких доработок. Флаг включается после
  подтверждения стороной 1С (строгое проведение, отключение фолбэка,
  допустимость более пяти документов). Триггер из карточки org-07
  «один заказ = один склад» наступил — см.
  [Заказы → Фиксация склада сайтом](rules/orders.md).
