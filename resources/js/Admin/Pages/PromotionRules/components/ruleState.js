...
export const emptyReward = () => ({
    ...
    multiply: 'once',
    max_multiplier: 1,
    // Платная промо-позиция по умолчанию отклоняемая — тот же дефолт, что и на
    // бэкенде. Иначе маркетолог ставит цену, не замечает выключенный тумблер,
    // и клиент получает в корзину платный товар, который не заказывал и убрать
    // не может.
    optional: true,
});
