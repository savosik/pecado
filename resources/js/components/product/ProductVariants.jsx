...
export default function ProductVariants({ variants = [], currentProductId, modelName = '' }) {
    ...
    /** Подпись варианта формируется на бэке в едином режиме для всей модели (attrs/name/sku). */
    const getVariantLabel = (variant) => variant.label || variant.sku || `#${variant.id}`;
                                ...
                                <Box flexShrink={0}>
                                    {variant.thumbnail || variant.main_image ? (
                                        <Box
                                            ...
                                            bg="gray.100" borderWidth="1px" borderColor="border"
