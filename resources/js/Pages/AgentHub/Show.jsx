...
import { FormField, MarkdownTextEditor, MarkdownView } from '@/Admin/Components';
...
export default function Show({ hub, topic, messages }) {
    useFlashToast();
    ...
    // иначе перезагрузка пропсов затирает набранный текст.
    ...
    const routeArgs = { token: hub.token, agentTopic: topic.id };
    ...
    const sendMessage = (e) => {
        e.preventDefault();
        post(route('agent-hub.topics.messages.store', routeArgs), {
            preserveScroll: true,
            onSuccess: () => reset(),
        });
    };
    ...
    const saveTask = (e) => {
        e.preventDefault();
        taskForm.put(route('agent-hub.topics.update', routeArgs), {
            preserveScroll: true,
    ...
    const passTurn = () => {
        router.post(route('agent-hub.topics.pass-turn', routeArgs), {}, { preserveScroll: true });
    };

    const closeTopic = () => {
        setCloseDialogOpen(false);
        router.post(route('agent-hub.topics.close', routeArgs), {}, { preserveScroll: true });
    };
        ...
        <>
            ...
            <Stack gap={4}>
                <Flex justify="space-between" align="center" gap={3} wrap="wrap">
                    <Text fontSize="xl" fontWeight="semibold">{topic.title}</Text>
                    <Button size="sm" variant="ghost" asChild>
                        <Link href={route('agent-hub.index', hub.token)}>← К списку топиков</Link>
                    </Button>
                </Flex>
                        ...
                        <Flex justify="space-between" align="center" wrap="wrap" gap={3}>
                            <HStack gap={3} wrap="wrap">
                                ...
                                {topic.created_by_agent && (
                                    <Text fontSize="sm" color="fg.muted">Завёл: {topic.created_by_agent}</Text>
                                )}
                            </HStack>
                            {!finished && (
                                <HStack gap={2}>
                ...
                <Card.Root>
                    ...
                    <Card.Body>
                        <Stack gap={4}>
                            {messages.length === 0 && (
                                <Text fontSize="sm" color="fg.muted">
                                    Сообщений пока нет. Первый ход — за {TURN_LABELS[topic.turn] ?? topic.turn}.
                                </Text>
                            )}
                            ...
                            {messages.map((message) => {
                                ...
                                const via = message.author === 'moderator' ? message.payload?.via : null;
                                ...
                                return (
                                    ...
                                    >
                                        <HStack gap={2} mb={2} wrap="wrap">
                                            ...
                                            {via && <Text fontSize="xs" color="fg.muted">({via})</Text>}
                            ...
                            {topic.status !== 'closed' && (
                                <form onSubmit={sendMessage}>
                                    <Stack gap={2}>
                                        <FormField
                                            label="Сообщение модератора"
                                            helpText="Видно обоим агентам, отправляется вне очереди и ход не передаёт."
                                        >
                                            <Textarea
                                                value={data.body}
...
Show.layout = (page) => <HubLayout hub={page.props.hub}>{page}</HubLayout>;
