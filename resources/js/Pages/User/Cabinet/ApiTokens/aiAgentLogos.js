/**
 * Знаки ИИ-агентов для баннера подключения MCP.
 *
 * Пути SVG взяты из simple-icons 16.31.0 (CC0) без изменений — логотипы не
 * перерисованы по памяти. Знаков OpenAI и Сбера в наборе нет: для ChatGPT и GigaChat
 * показывается нейтральная монограмма с названием, а не имитация чужого логотипа.
 * hex = null — чёрный знак, в компоненте берёт цвет текста темы.
 *
 * Товарные знаки принадлежат владельцам и используются только чтобы показать,
 * с какими агентами работает подключение.
 *
 * В баннер попадают только агенты, которые реально подключают наш сервер со
 * статическим ключом (проверено по документации вендоров 16.09.2026). GigaChat и
 * DeepSeek убраны: их приложения подключать пользовательский MCP-сервер не умеют.
 * Если вендор добавит поддержку — вернуть знак в группу, данные остаются в AI_AGENT_ICONS.
 */
export const AI_AGENT_ICONS = {
    "Claude": {
        "name": "Claude",
        "slug": "claude",
        "hex": "#D97757",
...
export const AI_AGENT_GROUPS = [
    { region: "США", items: [AI_AGENT_ICONS["Claude"], AI_AGENT_ICONS["ChatGPT"], AI_AGENT_ICONS["Gemini"], AI_AGENT_ICONS["Copilot"], AI_AGENT_ICONS["Cursor"]] },
