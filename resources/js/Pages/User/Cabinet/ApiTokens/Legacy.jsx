import {
    Accordion, Box, Flex, Text, Card, HStack, VStack, Badge, Heading, Code,
} from '@chakra-ui/react';
import { Head, usePage } from '@inertiajs/react';
import CabinetLayout from '../CabinetLayout';
import CopyableUrl from './CopyableUrl';
import {
    LuShieldCheck, LuCode, LuPackage, LuDollarSign, LuWarehouse, LuShoppingCart,
    LuTriangleAlert, LuArrowRightLeft, LuTruck, LuFileText, LuTrash2, LuCheck, LuClock,
} from 'react-icons/lu';
...
/*  Страница Legacy API                             */
/* ──────────────────────────────────────────────── */
// Прежний формат с ключом в адресе: поддерживается бессрочно, но не развивается.
// Ключи создаются в разделе «API», здесь — только их адреса и описание методов.
export default function Legacy({ tokens = [] }) {
    ...
    return (
        <CabinetLayout title="Legacy API">
            <Head title="Legacy API — Pecado" />
            ...
            <VStack align="stretch" gap="3" mb="6">
                ...
                >
                    ...
                    <Text fontSize="xs" color="amber.700" _dark={{ color: 'amber.300' }}>
                        Прежний формат для уже настроенных интеграций: поддерживается, но не развивается.
                        Новые подключения делайте через API v1 в разделе «API». Любой, кто знает адрес с ключом,
                        получает доступ к вашим ценам и может создавать заказы.
                    </Text>
                </HStack>
