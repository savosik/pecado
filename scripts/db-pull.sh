...
# приходится 6395 MB, то есть 96 %. Полезных данных — около 240 MB.
#
...
#
# Тянуть их целиком — значит ждать часы ради данных, которые локально
# бесполезны. `--full` отключает фильтр, если понадобится точная копия.
SKIP_DATA_TABLES=(
