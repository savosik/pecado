...
### Keyword-first: как устроена релевантность

Пользовательский поиск (`/search`, дропдаун, `/api/content/products/search`) работает по принципу **keyword-first**:

1. Сначала выполняется **чистый полнотекстовый** поиск (без hybrid-опций).
2. Только если он вернул меньше `SEARCH_HYBRID_FALLBACK_MIN` результатов (по умолчанию 1) — запрос **повторяется с семантикой** (hybrid). Это fallback для опечаток/синонимов.

Зачем: гибрид с `semanticRatio` подмешивает семантических «соседей» и может вытеснить точные совпадения (или совсем исключить документ без вектора). Keyword-first гарантирует, что точные/подстрочные совпадения никогда не тонут, а семантика включается только когда полнотекст ничего не нашёл. Опции гибрида и решение о fallback — в `app/Support/Search/HybridSearchOptions.php`. Подбор «похожих товаров» (`SimilarProductsService`) семантику использует всегда — там это уместно.

---

## Векторы (эмбеддинги) — аудит и ремонт

**Симптом:** товар находится в каталоге (`/products`), но НЕ находится в строке поиска / на `/search`. Причина — у документа нет `_vectors` (вектор не сгенерировался), а семантический fallback такой документ не видит.

Почему вектор мог не появиться:
- embedder в индексе не настроен (индекс сбрасывался, а `meilisearch:configure-embedders` не перезапускали);
- OpenRouter вернул ошибку (`402 Insufficient credits`/троттлинг) во время индексации — документ лёг без вектора, ретрая нет.

**Команда `search:repair-embeddings`** — аудит и точечный ремонт:

```bash
# Только отчёт: сколько документов без вектора, настроен ли embedder, есть ли упавшие задачи
docker exec pecado-app php artisan search:repair-embeddings --dry-run

# Ремонт: при необходимости настроить embedder + переиндексировать ТОЛЬКО товары без вектора
docker exec pecado-app php artisan search:repair-embeddings --reindex-missing
```

Ремонт дёшев по кредитам: товары, у которых вектор уже есть, не трогаются (в отличие от полного `search:sync`). Если отчёт показывает упавшие задачи `Insufficient credits` — сначала пополни баланс OpenRouter, потом запускай `--reindex-missing`.

Команда стоит в планировщике (`search:repair-embeddings --reindex-missing`, ежедневно в 03:20) — досчитывает векторы товарам, у которых эмбеддинг падал по кредитам, когда баланс снова есть.
