...
- [ ] Поиск находит реализацию по части номера (с дефисом и без)
- [ ] Поиск находит реализацию по названию товара в составе
- [ ] Поиск находит реализацию по `sku` товара (точное/префиксное)
- [ ] Поиск по штрихкоду — точный матч
- [ ] В подсказке отображается номер, дата, сумма
- [ ] При совпадении по составу видно, какой товар найден
- [ ] Бейдж «⚠ N открытых возвратов» показан, если есть
- [ ] Дедупликация (реализация в выдаче один раз)
- [ ] Сортировка: точные по номеру → fuzzy по составу
- [ ] Лимит 20 сохранён
- [ ] Покрыто feature-тестом (`tests/Feature/User/ReturnShipmentPickerTest.php`)
