...
- [ ] Поиск находит товар по бренду
- [ ] Точный матч по 13-значному штрихкоду — товар в выдаче первым
- [ ] При ровно 13 цифрах + точном штрихкоде — авто-вставка без выпадайки (опция включаема)
- [ ] Поиск по `sku` с дефисом — точное и префиксное
- [ ] В строке автокомплита показывается бейдж «✓ покупали N×»
- [ ] Бейдж «★ в избранном» работает
- [ ] Fuzzy для русских опечаток в названии (`кросовки` → находит «Кроссовки»)
- [ ] Fuzzy для опечаток в бренде (`addidas` → «Adidas»)
- [ ] Точные совпадения по идентификаторам выше fuzzy в выдаче
- [ ] Лимит 15 сохранён
- [ ] Покрыто feature-тестом (`tests/Feature/User/CabinetCartProductSearchTest.php`)
