...
- [ ] Поиск находит возврат по части номера (с дефисом и без)
- [ ] Поиск находит возврат по номеру исходной реализации
- [ ] Поиск находит возврат по названию товара в составе (LIKE + fuzzy)
- [ ] Поиск находит возврат по бренду в составе (fuzzy)
- [ ] Поиск по 13-значному штрихкоду — точный матч
- [ ] Поиск по `sku` — точное/префиксное
- [ ] Поиск по тексту комментария причины
- [ ] Multi-select по причине: чекбоксы вместо single-select
...
- [ ] Покрыто feature-тестами (`tests/Feature/User/ReturnSearchTest.php`)
