...
- [ ] Поиск по бренду в составе (LIKE и fuzzy)
- [ ] Поиск по штрихкоду — точный матч
- [ ] Поиск по `code` товара
- [ ] Поиск отгрузок по UUID связанного заказа (`?order_uuid=...`)
- [ ] Фильтр по бренду (multi-select)
- [ ] Фильтр по категории (с поддержкой подкатегорий)
- [ ] Фильтр по складу/региону (если поле существует)
- [ ] Multi-select по статусу
- [ ] Debounce 400 мс
- [ ] Дедупликация
- [ ] Fuzzy для названия товара / бренда (русские опечатки)
- [ ] Кнопка «Все отгрузки» на странице заказа
- [ ] Покрыто feature-тестами (`tests/Feature/User/ShipmentSearchTest.php`)
