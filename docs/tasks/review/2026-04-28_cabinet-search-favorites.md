...
### Этап 1 (PR 3.2) — закрыто

- [x] Поиск находит товар в избранном по названию (LIKE)
- [x] Поиск по бренду
- [x] Точный матч по штрихкоду (через `QueryRouter::TYPE_BARCODE`)
- [x] Поиск по `sku` / `code` (LIKE)
- [x] Сортировка по дате добавления (default), цене ↑/↓, имени ↑/↓, наличию
- [x] Фильтр «в наличии» использует региональные склады пользователя (`whereExists` по primary)
- [x] Фильтр «предзаказ» (preorder склады)
- [x] Фильтр «нет в наличии»
- [x] Multi-select по бренду (фасет — только бренды из текущего избранного)
- [x] Multi-select по категории
- [x] Группировка по категориям (клиентская — toggle, без обращения к серверу)
- [x] Debounce 400 мс на поиске
- [x] Чипы активных фильтров + сброс (общий `<SelectedFilters />`)
- [x] История поиска через `useSearchHistory('favorites')` + `<datalist>`
- [x] Покрыто feature-тестом (`tests/Feature/User/FavoriteSearchTest.php`, 15 кейсов)

### Этап 2 (PR 4.x, Meilisearch fuzzy за флагом) — отдельный PR
