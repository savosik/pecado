...
- [x] Фильтр по диапазону даты загрузки работает (`date_from`, `date_to`)
- [x] Фильтр по размеру (МБ) работает (`size_from_mb`, `size_to_mb`)
- [ ] (Опционально, отложено) Фильтр по разрешению изображения — `width`/`height` доступны не для всех файлов в `custom_properties`, нужна отдельная задача
- [x] Fuzzy для названий товаров через Meilisearch (за флагом `CABINET_SEARCH_FUZZY_PRODUCTS`)
- [ ] (Отложено) Fuzzy для названий брендов — модель `Brand` не имеет Searchable trait, нужна отдельная индексация
- [x] Exact-поиск по `sku`/`code`/`barcode`/`external_id` сохранён
- [x] LIKE для статей/новостей/историй сохранён
- [x] Чипы фильтров обновлены (4 новых: даты, размеры)
- [x] Покрыто feature-тестом (`tests/Feature/User/MediaSearchTest.php`, 8 кейсов)
