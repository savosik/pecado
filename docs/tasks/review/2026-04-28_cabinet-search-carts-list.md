...
### Этапы 1–3 (PR 6.1) — закрыто

- [x] Поиск находит корзину по имени (LIKE)
- [x] Поиск находит корзину по товару в составе (LIKE по name/sku/code/brand + точный по barcode для штрихкода)
- [x] Сортировка работает по 5 полям (`updated_at`/`created_at`/`name`/`total_amount`/`items_count`) с `sort_order=asc/desc`
- [x] Фильтр по сумме от/до (subquery `SUM(quantity * price)` с CAST AS REAL для SQLite)
- [x] Фильтр по количеству позиций от/до (subquery `COUNT(*)`)
- [x] Чекбокс «Только пустые» (`doesntHave('items')`)
- [x] Чекбокс «Только активная» (`is_active = true`)
- [x] Debounce 400 мс на поиске
- [x] Бейдж активных фильтров (`<SelectedFilters />` + history через `useSearchHistory('carts')`)
- [x] Дедупликация (`whereHas` гарантирует один ряд на корзину)
- [x] Scope: только свои корзины (`carts.user_id = $user->id`)
- [x] `match_source` (`name` / `composition`) с бейджем «по составу» в UI
- [x] Покрыто feature-тестами (`tests/Feature/User/CabinetCartListSearchTest.php`, 15 кейсов)

### Этап 4 (опционально) — отдельный PR

- [ ] Fuzzy через Meilisearch для `Cart` (если объёмы вырастут)
