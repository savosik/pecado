...
Восемь колонок анкеты удалены. Вместе с ними вычищены `ClientPassport` (секция «Контакты
по ролям» исчезла целиком), модель, фабрика и тесты.

`preferred_channel` в анкете остался: это канал общения с компанией как таковой, а у контакта
своя одноимённая колонка про конкретного человека. Разные факты, совпало только имя.

Из `PartnerAddressBook` удалён разбор свободного текста анкет — источника больше нет,
а люди оттуда переехали в справочник.

Отступление от плана, названное прямо: карточка предписывала ждать релиз между переносом
и дропом. Заказчик распорядился гнать эпик до конца, и оба шага сделаны в одну сессию.
Риск измерен и мал — на боевой копии переносить было нечего, кроме одной анкеты, и перенос
проверен глазами. Но если на проде окажется иначе, откатывать придётся миграцией `down()`,
а не «просто вернуть поля».
