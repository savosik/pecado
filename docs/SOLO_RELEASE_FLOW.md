...
## Прод не гоняет тесты дважды

Релиз идёт через `git push origin dev:main`, то есть на прод приезжает **ровно тот же
коммит**, который перед этим позеленел на dev. Прогонять на нём тот же test suite второй
раз — лишние минуты простоя релиза.

Job `check-dev` в `deploy-prod.yml` пропускает тесты, если по этому же SHA выполнены
**все три** условия:

1. Есть завершённый прогон `deploy-dev`, и он успешен.
2. В нём job **«Lint & Tests» именно `success`, а не `skipped`**. Это ключевое: на dev
   работает fast-lane, и при правках только в md/css тесты там не запускаются — зелёный
   workflow сам по себе ничего не доказывает.
3. В нём job **«Deploy to Dev» тоже `success`** — код не просто собрался, а доехал и
   поднялся на dev-сервере.

Любое сомнение (прогон не найден, ещё идёт, тесты пропущены, деплой упал) → тесты
гоняются полностью. Причина решения пишется в лог прогона как `notice`.

**Что это даёт по времени.** Замер трёх последних прод-релизов:

| Job | Время | На критическом пути? |
|---|---|---|
| Lint & Tests | 6м32с – 7м42с | да, самый долгий |
| Build Frontend | 2м49с – 3м01с | идёт параллельно тестам |
| Deploy to Production | 6м55с – 8м07с | да |

Тесты и сборка идут параллельно, поэтому путь был `max(7, 3) + 7 ≈ 14 мин`, а станет
`3 + 7 ≈ 10 мин`. **Экономия ~4–5 минут на релиз**, не больше: сборку фронтенда
пропустить нельзя, её артефакты нужны для деплоя.

> ⚠️ Работает только если dev-прогон **успел завершиться** до пуша в main. `make release`
> это гарантирует — он ждёт зелёного dev перед `push dev:main`. При ручном пуше двух веток
> подряд dev ещё будет идти, `check-dev` не найдёт завершённого прогона, и тесты
> прогонятся заново. Это безопасный фолбэк, а не поломка.
...
`[fast]` в конце сообщения пропускает тесты **только на dev** (deploy-dev.yml).

На проде тесты пропускаются лишь автоматически и лишь при полном совпадении условий —
вручную это не включается (см. «Прод не гоняет тесты дважды» ниже).
