#!/usr/bin/env bash
# Создаёт топик Agent Hub на проде из topic-ship-together-trials.md (первая строка — название, остальное — постановка).
# Запуск: bash docs/ship-together/agent-hub/create-topic.sh
set -euo pipefail
cd "$(dirname "$0")"
TITLE_B64=$(head -n1 topic-ship-together-trials.md | base64 -w0)
BODY_B64=$(tail -n +3 topic-ship-together-trials.md | base64 -w0)
