...
│  • Environment `production`: только ветка main, approve снят    │
...
3. Деплой идёт **сразу** после зелёных тестов и сборки — ручной approve в Environment `production` снят 2026-08-20.
4. Передумать можно только нажав **Cancel run**, пока идут тесты и сборка.
...
git push origin main
# → запускается deploy-prod.yml → откат уезжает на прод автоматически
```
...
- [ ] Сделать тестовый релиз (`push dev:main`) с минимальной правкой → пройти весь цикл → проверить что прод обновился
...
О: Тогда merge не повлияет на прод — деплой не стартует без зелёных тестов и сборки. Откатить нечего. Нужно: 1) понять, почему упало, 2) исправить в `dev`, 3) повторить релиз.

**В: Что делать, если на main CI прошёл, но деплой на проде упал?**
...
О: 1) Включить уведомления о падении деплоя в Telegram (TODO в workflow). 2) Дать 1-2 коллегам admin-доступ к репозиторию, чтобы было кому выкатить фикс. 3) Настроить UptimeRobot для алерта на падение `https://pecado.ru/up`.
