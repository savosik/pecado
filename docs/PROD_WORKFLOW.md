...
> ⚠️ **С 2026-05-09 dev и prod живут на одном внешнем IP `93.94.150.16`.** Prod на стандартных портах `22/80/443/15672`, dev — на временно проброшенных `8022/8080/8443/25672` (см. [DEV_SERVER_CREDENTIALS.md](./DEV_SERVER_CREDENTIALS.md)). Цикл `local → dev → main → prod` работает целиком, просто dev адресуется по альтернативным портам.
...
│  • домен: http://loc.pecado.ru                                   │
└──────────────────────────────┬──────────────────────────────────┘
                               │ git push origin dev
                               ▼
┌─────────────────────────────────────────────────────────────────┐
│  DEV — m-s-site (10.2.2.100), общий IP 93.94.150.16             │
│  • временные порты: 8022 (ssh), 8080/8443 (http/s), 25672 (mq)  │
...
└──────────────────────────────┬──────────────────────────────────┘
                               ...
                               ▼
┌─────────────────────────────────────────────────────────────────┐
│  PROD — m-s-web (10.2.2.101), общий IP 93.94.150.16             │
...
| `dev` | Текущая разработка, все фичи | dev-сервер `m-s-site` (доступ через временные порты на `93.94.150.16`) | Разработчик (push или merge feature/) |
