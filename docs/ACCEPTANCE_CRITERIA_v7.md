# Критерии приёмки интеграции 1С (ERP) ↔ Сайт Pecado | v7.2
...
```json
{
  ...
  "file_url": "s3://prices-exchange/2026-03-26/partner_a1b2c3d4.csv",
  "records_count": 50000,
...
- Фоновое задание собирает пул изменений, группирует по партнёрам, формирует CSV-файл на каждого партнёра, загружает в MinIO, отправляет по уведомлению на каждый файл

#### Полная ночная выгрузка (Full Dump)
- **Расписание:** Один раз в сутки (03:00)
- Полный пересчёт всех цен, файл на каждого партнёра
- `upload_type: "full"` — Laravel заменяет все цены указанного партнёра
...
prices-exchange/
  2026-03-26/
    partner_a1b2c3d4.csv    ← дельта или full для партнёра a1b2c3d4
    partner_f9e8d7c6.csv    ← дельта или full для партнёра f9e8d7c6
    ...
```
...
3. **Job** скачивает CSV из MinIO, резолвит `partner_uuid` → `partner_id`, загружает маппинг UUID→INT для товаров/складов, читает CSV потоково (`fgetcsv`), батч-вставка по 5000 строк
4. **Стратегия обновления:** один файл = полный прайс-лист одного партнёра. При обработке выполняется полное удаление всех цен партнёра (`DELETE WHERE partner_id = ?`), затем вставка нового прайс-листа батчами (`INSERT` по 5000 строк). Это быстрее построчного `ON DUPLICATE KEY UPDATE`.
...
- [ ] При обработке файла: **полное удаление** всех цен партнёра (`DELETE WHERE partner_id = ?`) → вставка нового прайс-листа батчами. Один файл = полный набор цен одного партнёра.
