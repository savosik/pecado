# Критерии приёмки интеграции 1С (ERP) ↔ Сайт Pecado | v7.0
...
1. **Таблица `individual_prices`** — MySQL, композитный PK `(partner_uuid, product_uuid, warehouse_uuid)`, поле `price` DECIMAL(10,2)
2. **Consumer** слушает `individual_prices.ready` в очереди `erp_in.prices`
3. **Job** скачивает JSONL из MinIO, читает потоково (генераторы PHP), батч-вставка `INSERT ... ON DUPLICATE KEY UPDATE` по 2000–5000 строк
