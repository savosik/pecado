...
4. **Стратегия обновления:** `DELETE WHERE partner_id = ?` → `INSERT` (один файл = один партнёр, замена всех цен партнёра)
...
- [ ] При обработке файла: удаление старых цен партнёра → вставка новых (`DELETE → INSERT`).
