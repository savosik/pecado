# Критерии приёмки интеграции 1С (ERP) ↔ Сайт Pecado | v7.1
...
| `first_name` | string \| null | — | Имя (разбор из НаименованиеПолное) |
...
- [ ] Сообщение содержит `first_name`, `last_name`, `middle_name` (разбор ФИО из полного имени).
...
| `file_url` | string | ✅ | Путь к файлу в MinIO (S3) |
...
#### Инкрементальная выгрузка (дельта)
- **Расписание:** Каждые 15–30 минут рабочего дня
- При изменении цены, скидки, курса валюты или параметров партнёра — 1С записывает объект в регистр `ОчередьВыгрузкиИндивидуальныхЦенPecado`
- Фоновое задание собирает пул изменений, рассчитывает цены, формирует JSONL файл, загружает в MinIO, отправляет уведомление в RabbitMQ

#### Полная ночная выгрузка (Full Dump)
- **Расписание:** Один раз в сутки (03:00)
- Полный пересчёт всех 40 млн цен
- `upload_type: "full"` — Laravel полностью заменяет данные в таблице

### Обработка на стороне сайта (Laravel)

1. **Таблица `individual_prices`** — MySQL, композитный PK `(partner_id, product_id, warehouse_id)` (INT UNSIGNED), поле `price` DECIMAL(15,2). UUID→INT резолвинг выполняется при импорте через маппинг `erp_id → users.id`, `external_id → products.id`, `external_id → warehouses.id`
2. **Consumer** слушает `individual_prices.ready` в очереди `erp_in.prices`
3. **Job** скачивает JSONL из MinIO, загружает маппинг UUID→INT в память, читает потоково (генераторы PHP), резолвит UUID→INT, батч-вставка `INSERT ... ON DUPLICATE KEY UPDATE` по 5000 строк
4. При `upload_type: "full"` — полная замена данных (временная таблица + RENAME или удаление старых)
5. **Очистка дампов:** Artisan-команда `app:clean-price-dumps` удаляет файлы из MinIO старше 3 дней, запуск ежедневно в 04:00

### Критерии приёмки

- [ ] 1С формирует JSONL файл с индивидуальными ценами и загружает в MinIO (бакет `prices-exchange`).
- [ ] 1С отправляет `individual_prices.ready` через `erp.events` с routing key `individual_prices.ready`.
- [ ] Дельта-выгрузка запускается каждые 15–30 минут при наличии изменений.
- [ ] Полная выгрузка запускается ежедневно в 03:00 с `upload_type: "full"`.
- [ ] Сайт принимает `individual_prices.ready` и загружает данные в таблицу `individual_prices`.
- [ ] Потоковое чтение JSONL (без загрузки всего файла в RAM).
- [ ] Батч-вставка через `INSERT ... ON DUPLICATE KEY UPDATE`.
- [ ] При `upload_type: "full"` — полная замена данных в таблице.
- [ ] Каталог отдаёт индивидуальную цену авторизованного пользователя (JOIN из `individual_prices`).
- [ ] Файлы дампов автоматически удаляются из MinIO через 3 дня.
