# Критерии приёмки интеграции 1С (ERP) ↔ Сайт Pecado | v10.0
...
5. **ОБНОВЛЕНО: US-02 (Партнёры)** — добавлен реквизит `client_status` в `partner.created` (1С → Сайт). _Источник: решение по нормализации._
...
| `client_status` | string | ✅ | **v10** Статус клиента: `"active"`, `"closed"`. По умолчанию `"active"` |
...
> **v10:** Поле `client_status` передаёт статус партнёра. Партнёры с `client_status = "closed"` отображаются на сайте как неактивные (не могут оформлять заказы).
...
- [ ] **v10:** Сообщение содержит `client_status` (`"active"` или `"closed"`).
...
## US-18: Клиентские сегменты и статусы (NEW v10)

**Область:** Управление клиентской базой в 1С + отображение на сайте

> Источник: митинг 07.04.2026

### Сегменты клиентов

| Уровень | Рабочее название | Скидка | Описание |
|---|---|---|---|
| 1 | Silver | 10% | Базовый уровень |
| 2 | Gold | 15% | Средний уровень |
| 3 | Diamond | 20% | Максимальный типовой уровень |
| 4 | Индивидуальный | Разная | Только для клиентов с деревом контрагентов |

> Окончательные названия уровней (Black/White/Red или другие) — **открытый вопрос**, ответственная: Лена.

### Отображение статуса в ЛК

- Наименование типового соглашения = название клиентского сегмента.
- Сайт отображает «плашку» со статусом в личном кабинете партнёра.
- Статус определяется из наименования активного типового соглашения партнёра.
...
- [ ] Сайт отображает статус клиента (наименование соглашения) в личном кабинете.
- [ ] Смена соглашения (например, с 10% на 15%) — это смена одного типового соглашения на другое.
