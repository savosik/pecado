...
> **Версия:** 4.0 (обновлено по итогам реализации 20 марта 2026)
> **Предыдущая версия:** v3 (обновлено по итогам планёрки 17 марта 2026)
>
> **Изменения v4:**
...
> - `(v2)` / `(v3)` — рядом с изменённым полем в JSON
...
> | `model` | `{ uuid, name }` \| null | Настраиваемый доп. реквизит | ✏️ v4 Модель — для объединения вариантов товара (напр., цвета). UUID доп. реквизита задаётся в `Константы.НастройкиОбменаPecado`. |
>
...
- [ ] ✏️ v4 Поле `model` — nullable объект `{ uuid, name }`. Источник — настраиваемый доп. реквизит, UUID которого задаётся в `Константы.НастройкиОбменаPecado`.
...
| 3 | Формат банковских реквизитов в `contractor.created` — нужны ли `bank_accounts`? | Не уточнено | — |
