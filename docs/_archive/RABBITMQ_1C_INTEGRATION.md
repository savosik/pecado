...
|---|---|---|
| `erp_in.partners` | `partner.*`, `contractor.*` | Управление партнёрами и контрагентами |
...
#### `partner.created` — Создание/активация партнёра (v4)

**Routing key**: `partner.created`  
**Очередь**: `erp_in.partners`  
**Действие**: Если пользователь с таким email существует — активирует и привязывает `erp_id`. Если нет и есть `password` — создаёт нового пользователя с обязательной сменой пароля.

```json
{
  "event": "partner.created",
  "message_id": "msg-partner-001",
  "uuid": "erp-id-партнёра-uuid",
...
| Поле | Тип | Обязательно | Описание |
|---|---|---|---|
| `uuid` | `string` | ✅ | ERP ID партнёра |
| `login` | `string` | ✅ | Email пользователя (используется для поиска) |
| `password` | `string` | нет | Временный пароль (обязателен при создании нового пользователя). Пользователь будет обязан сменить пароль при первом входе |
| `name` | `string` | нет | ФИО партнёра |
| `phone` | `string` | нет | Телефон |
| `email` | `string` | нет | Email |

---

#### `contractor.created` — Создание/обновление контрагента (v4)

**Routing key**: `contractor.created`  
**Очередь**: `erp_in.partners`  
**Действие**: Создаёт или обновляет контрагента (компанию) с привязкой к партнёру по `partner_uuid`. Идемпотентно.

```json
{
  "event": "contractor.created",
  "message_id": "msg-contractor-001",
  "uuid": "uuid-контрагента-в-1с",
  "partner_uuid": "erp-id-партнёра",
  "name": "ООО «Рога и Копыта»",
  "legal_name": "Общество с ограниченной ответственностью «Рога и Копыта»",
  "tax_id": "7701234567",
...
| Поле | Тип | Обязательно | Описание |
|---|---|---|---|
| `uuid` | `string` | ✅ | UUID контрагента в 1С (записывается как `erp_id` в `Company` на сайте) |
| `partner_uuid` | `string` | ✅ | ERP ID партнёра (для привязки к пользователю). Если партнёр не найден — событие пропускается |
| `name` | `string` | нет | Название контрагента |
| `legal_name` | `string` | нет | Полное юридическое название |
| `tax_id` | `string` | нет | ИНН |
| `registration_number` | `string` | нет | КПП / ОГРН |
| `legal_address` | `string` | нет | Юридический адрес |
| `actual_address` | `string` | нет | Фактический адрес |
| `phone` | `string` | нет | Телефон |
| `email` | `string` | нет | Email |
| `country` | `string` | нет | Код страны (`RU`, `BY`, `KZ`). По умолчанию `BY` |

> ℹ️ Партнёр должен быть создан **до** контрагента (через `partner.created`). Если партнёр с указанным `partner_uuid` не найден на сайте — событие пропускается.
...
| `brand` | `object\|string\|null` | нет | **v4**: объект `{uuid, name}`. Для обратной совместимости принимает и строку |
| `brand.uuid` | `string` | ✅* | UUID бренда в 1С (записывается как `external_id`) |
| `brand.name` | `string` | ✅* | Название бренда |
...
> ❗ **v4 изменение**: Атрибуты теперь **мержатся** с существующими (добавляются/обновляются), а не заменяют их полностью. Не переданные атрибуты остаются без изменений.
