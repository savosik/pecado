...
> **Версия:** 3.0 (обновлено по итогам планёрки 17 марта 2026)
> **Предыдущая версия:** v2 (обновлено по итогам встречи ~10-11 февраля 2026)
>
> **Изменения v3:**
...
|---|---|---|---|
| `erp_in.partners` | `partner.*` | `partner.deleted` | Деактивация пользователей |
...
> [!WARNING]
> **Партнёр в 1С** = физическое лицо (контактное лицо). **Контрагент** = юридическое лицо (компания). Поиск партнёра при `partner.created` выполняется по полю `email` через контактную информацию (вид КИ: `EmailПартнера`).
...
- [ ] Поле `model` — nullable объект `{ uuid, name }`. Если в 1С значение строковое — маппится в объект при отправке.
