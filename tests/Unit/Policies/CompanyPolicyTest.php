<?php

namespace Tests\Unit\Policies;

use App\Models\Company;
use App\Models\User;
use App\Policies\CompanyPolicy;
use PHPUnit\Framework\TestCase;
use PHPUnit\Framework\Attributes\Test;

class CompanyPolicyTest extends TestCase
{
    private CompanyPolicy $policy;

    protected function setUp(): void
    {
        parent::setUp();
        $this->policy = new CompanyPolicy();
    }

    #[Test]
    public function any_user_can_view_any_companies(): void
    {
        $user = $this->createMock(User::class);
        $user->method('__get')->willReturnCallback(fn($key) => $key === 'is_admin' ? false : null);

        $this->assertTrue($this->policy->viewAny($user));
    }

    #[Test]
    public function admin_can_view_any_company(): void
    {
        $admin = $this->createMock(User::class);
        $admin->method('__get')->willReturnCallback(fn($key) => match($key) {
            'is_admin' => true,
            'id' => 1,
            default => null
        });

        $company = $this->createMock(Company::class);
        $company->method('__get')->willReturnCallback(fn($key) => $key === 'user_id' ? 2 : null);

        $this->assertTrue($this->policy->view($admin, $company));
    }

    #[Test]
    public function user_can_view_own_company(): void
    {
        $user = $this->createMock(User::class);
        $user->method('__get')->willReturnCallback(fn($key) => match($key) {
            'is_admin' => false,
            'id' => 1,
            default => null
        });

        $company = $this->createMock(Company::class);
        $company->method('__get')->willReturnCallback(fn($key) => $key === 'user_id' ? 1 : null);

        $this->assertTrue($this->policy->view($user, $company));
    }

    #[Test]
    public function user_cannot_view_other_user_company(): void
    {
        $user = $this->createMock(User::class);
        $user->method('__get')->willReturnCallback(fn($key) => match($key) {
            'is_admin' => false,
            'id' => 1,
            default => null
        });

        $company = $this->createMock(Company::class);
        $company->method('__get')->willReturnCallback(fn($key) => $key === 'user_id' ? 2 : null);

        $this->assertFalse($this->policy->view($user, $company));
    }

    #[Test]
    public function any_user_can_create_company(): void
    {
        $user = $this->createMock(User::class);
        $user->method('__get')->willReturnCallback(fn($key) => $key === 'is_admin' ? false : null);

        $this->assertTrue($this->policy->create($user));
    }

    #[Test]
    public function admin_can_update_any_company(): void
    {
        $admin = $this->createMock(User::class);
        $admin->method('__get')->willReturnCallback(fn($key) => match($key) {
            'is_admin' => true,
            'id' => 1,
            default => null
        });

        $company = $this->createMock(Company::class);
        $company->method('__get')->willReturnCallback(fn($key) => $key === 'user_id' ? 2 : null);

        $this->assertTrue($this->policy->update($admin, $company));
    }

    #[Test]
    public function user_can_update_own_company(): void
    {
        $user = $this->createMock(User::class);
        $user->method('__get')->willReturnCallback(fn($key) => match($key) {
            'is_admin' => false,
            'id' => 1,
            default => null
        });

        $company = $this->createMock(Company::class);
        $company->method('__get')->willReturnCallback(fn($key) => $key === 'user_id' ? 1 : null);

        $this->assertTrue($this->policy->update($user, $company));
    }

    #[Test]
    public function user_cannot_update_other_user_company(): void
    {
        $user = $this->createMock(User::class);
        $user->method('__get')->willReturnCallback(fn($key) => match($key) {
            'is_admin' => false,
            'id' => 1,
            default => null
        });

        $company = $this->createMock(Company::class);
        $company->method('__get')->willReturnCallback(fn($key) => $key === 'user_id' ? 2 : null);

        $this->assertFalse($this->policy->update($user, $company));
    }

    #[Test]
    public function admin_can_delete_any_company(): void
    {
        $admin = $this->createMock(User::class);
        $admin->method('__get')->willReturnCallback(fn($key) => $key === 'is_admin' ? true : null);

        $company = $this->createMock(Company::class);

        $this->assertTrue($this->policy->delete($admin, $company));
    }

    #[Test]
    public function user_can_delete_own_company(): void
    {
        $user = $this->createMock(User::class);
        $user->method('__get')->willReturnCallback(fn($key) => match($key) {
            'is_admin' => false,
            'id' => 1,
            default => null
        });

        $company = $this->createMock(Company::class);
        $company->method('__get')->willReturnCallback(fn($key) => $key === 'user_id' ? 1 : null);

        $this->assertTrue($this->policy->delete($user, $company));
    }

    #[Test]
    public function user_cannot_delete_other_user_company(): void
    {
        $user = $this->createMock(User::class);
        $user->method('__get')->willReturnCallback(fn($key) => match($key) {
            'is_admin' => false,
            'id' => 1,
            default => null
        });

        $company = $this->createMock(Company::class);
        $company->method('__get')->willReturnCallback(fn($key) => $key === 'user_id' ? 2 : null);

        $this->assertFalse($this->policy->delete($user, $company));
    }
}
