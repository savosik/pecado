...
class ClientLastVisitTest extends TestCase
{
    ...
    {
        // Метка «сегодня» считается по календарным суткам, поэтому время
        // фиксируется. Дата именно абсолютная, а не `Carbon::today()`:
        // относительная всё равно вычисляется от настоящих часов машины,
        // и прогон в CI падал дважды подряд около полуночи по Москве.
