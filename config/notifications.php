    ...
    */
    ...
    'mail' => [
        ...
        */
        ...
        /*
        |----------------------------------------------------------------------
        | Получатели отчёта закупкам о повторных недоборах
        |----------------------------------------------------------------------
        |
        | Повторный недобор лечится запасом: еженедельный отчёт уходит закупщику
        | сам, менеджер продаж его не собирает. Пусто = отчёт не отправляется
        | (команда пишет об этом в вывод, а не молчит).
        |
        */

        'purchasing_recipients' => array_values(array_filter(array_map(
            'trim',
            explode(',', (string) env('MAIL_PURCHASING_RECIPIENTS', ''))
        ))),
        ...
        */
        ...
        |
        | Письма клиентам о заказах и возвратах здесь больше не гейтятся:
        | с note-10 их шлёт матрица уведомлений по настройке партнёра
        | (config/mail_occasions.php). Письмо менеджеру о новом заказе —
        | служебное, управляется матрицей сотрудника (staff.order_created).
        */
        'features' => [
