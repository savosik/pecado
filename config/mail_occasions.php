...
return [
    ...
    // Режим «Заказы в резерве» (v16.9.0, res-09). Исключение из общей тишины
    // клиентских уведомлений (решение 11.09.2026): без «резерв истекает» клиент
    // теряет удержание молча. Включено умолчанием, а не строками участникам —
    // повод рождается только у резервных заказов, то есть только у участников
    // режима; партнёр может отключить у себя в матрице.
    ...
    // Совместная отгрузка (v16.11.0, топик №7). Отказ группы приходит из 1С асинхронно,
    // клиент к тому времени закрыл кабинет — без письма он узнает, что заказы так и висят
    // в резерве, только по истечении срока. Повод есть только у участников режима резервов,
    // поэтому включён умолчанием как и остальные резервные. Одно письмо на группу.
    'orders.ship_together_rejected' => [
        'label' => 'Не удалось отгрузить заказы вместе',
        'subject' => 'Заказы {{order_number}}: отправить вместе не удалось, они остались в резерве',
        'default_destinations' => [['type' => 'login']],
        'default_enabled' => true,
        'client_visible' => true,
    ],
