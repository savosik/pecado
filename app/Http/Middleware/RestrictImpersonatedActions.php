...
/**
 * Что менеджеру нельзя, пока он смотрит сайт от имени клиента.
 *
 * Режим задуман как «посмотреть глазами клиента»: витрина, цены, корзина,
 * кабинет — всё доступно. Запрещено ровно то, что оставляет за клиентом след
...
class RestrictImpersonatedActions
{
    ...
    private const BLOCKED_ROUTES = [
        ...
        // и становится обязательством клиента, которого он не давал.
        'checkout.store',
        // Возврат — такой же документ от имени клиента.
        'cabinet.returns.store',
        // Обращение в поддержку: уходит письмом сотрудникам как вопрос клиента.
        'faq.questions.store',
        // Смена пароля закрыла бы клиенту вход на сайт.
        'cabinet.password.update',
        // Ключи интеграции клиента: выдача и отзыв ломают его выгрузки.
    ...
    public function handle(Request $request, Closure $next): Response
    {
        ...
        $message = 'Просмотр от имени клиента: оформлять документы и менять его доступы нельзя.';
