...
class TeamController extends CrmController
{
     ...
     * создаст заново, а закреплённые за ней клиенты потеряют менеджера.
    ...
    public function setActive(Request $request, PersonalManager $manager): RedirectResponse
    {
        ...
        // Клиенты остаются закреплёнными за скрытой карточкой — предупреждаем,
        // иначе их выручка молча выпала бы из разреза по менеджерам.
        if (! $isActive && $manager->users()->count() > 0) {
            $message .= '. За ней остались клиенты — переназначьте их в 1С, иначе в отчётах они останутся без менеджера.';
        }
