...
final class CrmEntityMap
{
     ...
     *
     * Партнёр и контрагент — то, ради чего справочник затевался; заказ,
     * реализация и лид добавились, когда модель обкаталась: водитель по
     * реализации и закупщик по заказу — живые роли, а не выдумка.
     *
     * Платёж, комментарий, задача, письмо, звонок и сам контакт исключены:
     * привязывать человека к платёжке бессмысленно, а «с кем говорили»
     * по звонку решается собственной колонкой `crm_calls.contact_id`.
     *
     * @return list<string>
     */
    public static function contactLinkableTypes(): array
    {
        return [self::CLIENT, self::CONTRACTOR, self::ORDER, self::SHIPMENT, self::LEAD];
    }
