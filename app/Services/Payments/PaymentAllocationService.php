...
class PaymentAllocationService
{
     ...
     *
     * Предоплата — сумма строк расшифровки с `target_type = order`. Поле
     * `shipments.paid_amount` она не трогает: оно остаётся строгой проекцией
     * расшифровки платежа по реализациям.
     *
     * Но график оплаты предоплата закрывает — иначе строка висела бы просроченной
     * навсегда. На проде так разнесена почти половина денег (39,7 из 82,8 млн ₽
     * на 2026-08-09), и без зачёта раздел «Финансы» показывал просрочку, которой
     * в учёте нет. Зачёт делает PaymentScheduleService в разрезе заказа.
     *
    ...
    public function recalculateOrders(array $orderUuids): void
    {
        ...
        foreach (array_chunk($orderUuids, 500) as $chunk) {
            ...
            // Строго после записи агрегатов, как и у реализаций: зачёт читает
            // суммы из БД, а не принимает их параметром.
            app(PaymentScheduleService::class)->applyOrderPrepayments($chunk);
        }
    }
