 ...
 */
class PaymentForecastService implements PaymentForecast
{
    ...
    /**
     * Балансы взаиморасчётов из 1С, сгруппированные по партнёру.
     *
     * 1С ведёт расчёты по КОНТРАГЕНТАМ, а у партнёра их бывает несколько (на проде
     * 234 контрагента на 144 партнёра). Поэтому строка нижнего уровня — контрагент,
     * а верхний нужен лишь чтобы имя партнёра не повторялось подряд без видимой
     * причины: суммы партнёра — это сумма его контрагентов, не отдельная величина.
     *
    ...
    public function balances(EloquentBuilder $clients): array
    {
        ...
        ContractorBalance::query()
            ...
            ->each(function (ContractorBalance $balance) use (&$grouped): void {
                ...
                // У партнёра показываем самую свежую дату из его контрагентов: 1С
                // обновляет их по отдельности, и старая дата означала бы, что устарел
                // весь баланс партнёра, — а это не так.
    ...
    /**
     * Партнёры с наибольшей просрочкой — блок «Кому звонить» на пульте.
     *
     * @param  EloquentBuilder<\App\Models\User>  $clients
     * @return list<array<string, mixed>>
     */
    ...
    /**
     * Оставить только просроченное.
     *
     * Колонка плановой даты у графика — `sch.due_date`; контроллер её больше
     * не называет, потому что у регистра она другая.
     */
    public function overdueOnly(QueryBuilder $query, ?CarbonImmutable $today = null): QueryBuilder
    {
        return $query->whereDate('sch.due_date', '<', ($today ?? CarbonImmutable::today())->toDateString());
    }

    public function dueBetween(QueryBuilder $query, string $from, string $to): QueryBuilder
    {
        return $query->whereBetween('sch.due_date', [$from, $to]);
    }
