...
class PlanProgressService
{
    ...
    /**
     * Разрез по менеджерам: план, факт, процент, прогноз и число активных клиентов.
     *
    ...
    /**
     * Клиенты скоупа: план, факт, процент и отставание — для таблицы «кого догонять».
     *
     * Сортировка по отставанию (`план − факт`), поэтому нужен факт по всем клиентам
     ...
     * что и у колонки «План / факт» в списке клиентов.
     *
     * @return list<array<string, mixed>>
     */
    public function clients(CarbonInterface $month, PlanScope $scope, User $actor, int $limit = 200): array
    {
        ...
        foreach ($planFact as $id => $cell) {
            ...
            // Клиенты без плана и без отгрузок — это просто остальная база
            ...
            $rows[] = [
                'id' => (int) $id,
                'name' => (string) ($names[$id] ?? ('Клиент #'.$id)),
                'plan' => $plan,
                'fact' => round($fact, 2),
     ...
     * суммы планов менеджеров (запас на новых клиентов), а менеджер — расписать
     * свою цифру по клиентам не полностью. Показываем расхождение с суммой и знаком.
     *
    ...
    /**
     * Факт и число активных клиентов в разрезе менеджеров.
     *
        ...
        return $this->cached('by-manager', $month, $scope, function () use ($scope, $month): array {
            ...
            foreach ($rows as $row) {
                if ($row['manager_id'] === null) {
                    continue;   // «Без менеджера» — в CRM таких клиентов нет по определению
                }
    ...
    /**
     * Контекст расчёта: клиенты скоупа, бизнес-дата 1С, рубли.
     */
