...
/**
 * Рабочий список клиентов: отбор, сортировка и сборка строки таблицы.
 *
...
class ClientListService
{
    ...
    /**
     * Запрос клиентов с применёнными фильтрами (без пагинации).
     *
     * @return Builder<User>
     */
    ...
    /**
     * Поиск по клиенту и по всему, что менеджеры о нём написали.
     *
     * Поиск по задачам сужен ровно так же, как лента и CrmTaskPolicy: без права
     * на весь отдел учитываются только свои задачи. Иначе клиент «всплывал» бы
    ...
    private function applySort(Builder $query, ClientListFilters $filters, User $actor): void
    {
        ...
        if ($filters->sortBy === 'next_task_due') {
            ...
            // Клиенты без срока — всегда в конце, независимо от направления:
    ...
    private function paginateByPlanPercent(Builder $query, ClientListFilters $filters): Paginator
    {
        ...
        usort($ids, function (int $a, int $b) use ($planFact, $desc): int {
            ...
            // Клиенты без плана — в конце при любом направлении: это не «ноль
    ...
    /**
     * Ближайшая незакрытая задача по каждому клиенту страницы.
     *
    ...
    /**
     * Последний комментарий по каждому клиенту страницы.
     *
     * Групповой максимум подзапросом: вытащить все комментарии и свернуть в PHP
     * означало бы тянуть переписку за годы ради одной строки на клиента.
     *
     * @param  list<int>  $clientIds
    ...
    private function row(
        ...
        // Клиент без профиля читается как активный — тот же дефолт, что в колонке БД.
    ...
    /**
     * Что показать мелкой строкой под именем клиента.
     *
