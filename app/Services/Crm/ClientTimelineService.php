...
class ClientTimelineService
{
    ...
    /**
     * Лента партнёра: записи менеджеров и документы партнёра в одной хронологии.
     *
     ...
     * и слить в памяти — на партнёре с длинной историей означала бы тянуть сотни
    ...
    public function forClient(
        ...
        if (in_array(self::TYPE_EMAIL, $wanted, true)) {
            // Черновики в ленту не идут: письмо становится событием в жизни партнёра,
        ...
        if (in_array(self::TYPE_CALL, $wanted, true)) {
            // Звонок доступен всем, кто видит партнёра, — как комментарий, а не как
            // задача: разговор с партнёром это общий факт, а не поручение.
