...
/**
 * Перенос управленческой таблицы продаж в CRM: планы и паспорта партнёров.
 *
 * Партнёры ищутся через {@see ClientNameIndex}: таблица и 1С называют одного и
 * того же партнёра по-разному, и сравнение строк как есть находит пятерых из
 * двухсот. Совпадение засчитывается, только когда кандидат ровно один — иначе
 * строка уходит в отчёт: ненайденного партнёра доводят руками, а вот неверно
...
class SalesSheetImporter
{
    ...
    public function import(
        ...
        // План менеджера собирается из планов его партнёров, поэтому сначала
        // раскладываем партнёров, а уже потом сводим итоги по менеджерам.
    ...
    /**
     * Партнёры, разложенные по именам, и они же по идентификаторам.
     *
    ...
    private function applyProfile(
        ...
        if ($attributes !== []) {
            $changes = $overwrite
                ? $attributes
                // Заполненное менеджером не трогаем: таблица описывает партнёра
    ...
    /**
     * План на ещё не привлечённых партнёров — в план менеджера, которому он поставлен.
     *
