...
/**
 * Приведение строк таблицы к партнёрам: слияние дублей и ключ для сравнения имён.
 *
 ...
 * на dev и на проде разный набор сопоставленных партнёров, и объяснить это
 * расхождение было бы нечем.
 */
trait MergesSheetClients
{
    /**
     * Строки одного партнёра, слитые в одну.
     *
     * В таблице партнёр иногда заведён дважды (два договора, переехавшая вниз
     * строка). Планы таких строк складываются: в CRM партнёр один, и его план —
    ...
    protected function groupByName(array $rows, SalesSheetImportReport $report): array
    {
        ...
        foreach ($rows as $row) {
            ...
            $report->warnings[] = sprintf(
                'Партнёр «%s» встречается в строках %d и %d — планы сложены.',
                $row->name,
                $previous->line,
                $row->line,
            );
     ...
     * строки таблицы описывают одного партнёра, когда названы одинаково, и
     * складывать планы разным партнёрам из-за общей фамилии недопустимо.
     */
