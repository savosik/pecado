...
/**
 * Разбор управленческой таблицы продаж (XLSX) в строки клиентов.
 *
...
class SalesSheetReader
{
    ...
    /** Подытоги: клиентами не являются и в базе их искать незачем. */
    private const TOTAL_ROWS = ['/^итого/u', '/^всего/u'];

    /** «Корзина» плана на ещё не привлечённых клиентов — уходит в план менеджера. */
    ...
    private function plans(Worksheet $sheet, int $line, array $planColumns): array
    {
        ...
        foreach ($planColumns as $month => $column) {
            ...
            // у клиентов, с которыми уже не работают. Планом он не становится.
    ...
    private function status(mixed $value): ?ClientLifecycleStatus
    {
        ...
        if ($status === null) {
            $this->warnings[] = "Неизвестный статус клиента «{$normalized}» — статус пропущен.";
        }
    ...
    private function businessType(mixed $value): ?BusinessType
    {
        ...
        if ($type === null) {
            $this->warnings[] = "Неизвестный тип клиента «{$normalized}» — тип пропущен.";
        }
