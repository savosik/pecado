...
/**
 * Партнёры: список, карточка, сводка по продажам.
 *
...
class ClientOperations
{
    ...
    public function show(User $actor, OperationInput $input): array
    {
        ...
        // Профиль — только с правом на него: «вижу партнёра» и «вижу, что
    ...
    /**
     * Сводка по продажам партнёра за период: деньги, помесячная динамика,
     * разрезы по брендам и категориям.
     *
     * Считает тот же `ShipmentAnalyticsService`, что и отчёт продаж, с контекстом
     * из одного партнёра — второго движка расчёта выручки в проекте нет.
     *
     * @return array<string, mixed>
     */
