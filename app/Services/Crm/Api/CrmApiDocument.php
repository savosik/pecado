 ...
 * из его сигнатуры аргументы тридцати разных вызовов нельзя — а в реестре
    ...
    private function description(): string
    {
        return implode("\n\n", [
            'API отдела продаж для ИИ-агентов менеджеров. Токен превращается в конкретного '
                .'сотрудника: операции идут его правами, и набор клиентов всегда ограничен его скоупом.',
            '## Что нужно знать заранее',
            '**Клиент** — пользователь с непустым персональным менеджером. Без менеджера это лид, '
            ...
            '**Лояльностью клиента владеет 1С** и перезаписывает её следующим обменом. Через API меняется '
                .'только жизненный статус — поле сайта, которое ведёт отдел продаж.',
            '**Чужой клиент даёт 404, а не 403** — существование записи вне скоупа не подтверждается.',
            ...
            'Всё, что создано через этот гейт, получает `source = agent`: в ленте клиента видно, что '
