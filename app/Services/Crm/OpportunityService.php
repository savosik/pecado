 ...
 * её в упорядоченный список партнёров, где у каждой строки написана причина
...
{
    /** Классы ABC в оценке: A-партнёр важнее C, но класс — не главный сигнал. */
    ...
    private function applyPreset(
        ...
        return match ($preset) {
            ...
            // Партнёр, заведённый на прошлой неделе, ещё не «ни разу не купил» —
     ...
     *
     * Медиана считается по партнёрам с покупками за год — сравнивать чек спящего
    ...
    private function score(array $rows, OpportunityPreset $preset, array $params): array
    {
        ...
        foreach ($rows as $row) {
            ...
            $signals = [
                ...
                // Просрочка в один полный цикл — уже максимум: партнёр, пропустивший
    ...
    /**
     * Почему партнёр в списке — словами, а не кодами сигналов.
     *
