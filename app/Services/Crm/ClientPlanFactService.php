...
/**
 * План и факт продаж по клиентам за месяц — для колонки в списке клиентов.
 *
 * Факт берётся исключительно из {@see ShipmentAnalyticsService}: свой SUM по
 * отгрузкам был бы вторым движком расчёта, и цифра в списке клиентов рано или
...
class ClientPlanFactService
{
     ...
     * восьмистам клиентам на каждое нажатие «отстают от плана» незачем.
     */
    ...
    /**
     * План и факт по конкретным клиентам за месяц.
     *
     * Два запроса независимо от числа клиентов: планы одной выборкой, факт —
     ...
     *
     * Отобрать «кто отстаёт» можно только зная факт по всем клиентам сразу:
    ...
    /**
     * Факт по клиентам за месяц, в рублях по бизнес-дате 1С.
     *
     * @param  list<int>  $clientIds
    ...
    private function facts(array $clientIds, CarbonInterface $month): array
    {
        ...
        // limit по числу клиентов: byPartner по умолчанию отдаёт топ-50, и без
