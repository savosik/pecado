 ...
 * АТС. Иначе резолв клиента, создание следующего шага и проверка доступа
...
class CrmCallService
{
     ...
     *
     * В отличие от задач — по клиентам, а не по участию: разговор с клиентом
     * видит всякий, кто видит клиента.
     *
    ...
    public function visibleTo(User $actor): Builder
    {
        ...
        // Менеджер без карточки клиентов не ведёт — но свои записи (звонок
     ...
     * из-за которой работа с клиентом и рассыпается.
     *
        ...
        return DB::transaction(function () use ($actor, $data, $related): array {
            ...
            if (is_array($followUpData) && ($followUpData['title'] ?? '') !== '') {
                $followUp = $this->tasks->create(
                    ...
                    // Привязку наследуем: следующий шаг по клиенту — шаг по тому же
                    // клиенту, искать его заново незачем.
                    $related,
                );
