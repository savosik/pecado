...
class ClientLifecycleService
{
    ...
    /**
     * Сменить тип аккаунта: партнёр ↔ сотрудник ↔ служебный ↔ удалён.
     *
     ...
     * а заодно спам, дубли и конкурентов. Причина обязательна не по формату,
     * а по смыслу — через полгода «почему этого нет в базе» спросят обязательно.
     *
     * Сотрудник и служебный перестают быть партнёрами для CRM (User::scopeClients),
     * а заказы, документы и вход в кабинет остаются как были. «Удалён» — мягкое
     * удаление: строка остаётся ради истории, но аккаунт скрыт из всех списков,
     * сессии и API-токены закрыты (User::revokeAccess), 1С его не воскрешает.
     * Вернуть можно тем же методом — из админки, где удалённые видны по фильтру.
     */
