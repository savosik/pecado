...
abstract class ExportField
{
    ...
    /**
     * Нативное значение поля для форматов с поддержкой структурированных
     * данных (JSON/XML). По умолчанию делегирует в getValue() — обратно
     * совместимо для всех скалярных полей. Multi-value поля переопределяют
     * этот метод, чтобы вернуть массив вместо склеенной строки.
     *
     * Why: CSV/XLS требуют строку в ячейке, а JSON/XML могут отдать массив
     * нативно. Раньше JSON-выгрузка отдавала `"111, 222"` вместо `["111","222"]`
     * — клиентам приходилось сплитить руками, что хрупко.
     */
    public function nativeValue(Product $product, ?User $clientUser = null): mixed
    {
        return $this->getValue($product, $clientUser);
    }
