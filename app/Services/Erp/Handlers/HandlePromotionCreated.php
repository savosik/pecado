...
/**
 * Обработка события promotion.created из 1С.
 * Создаёт ErpPromotion, привязывает товары по external_id и пересчитывает
 * у затронутых товаров соответствующий флаг (is_new / is_bestseller / is_liquidation).
 * Идемпотентен: повторный вызов с тем же uuid заменяет состав и type.
 */
class HandlePromotionCreated
{
    public function handle(array $payload): void
    {
        ...
        if (! $uuid || ! $type) {
            Log::warning('promotion.created: отсутствуют обязательные поля uuid/type', [
                'payload' => $payload,
            ]);
        ...
        if (! in_array($type, ErpPromotion::TYPES, true)) {
            Log::warning('promotion.created: неизвестный type', [
                'uuid' => $uuid,
                'type' => $type,
            ]);
        ...
        DB::transaction(function () use ($uuid, $type, $items) {
            ...
            if ($missing > 0) {
                Log::info('promotion.created: часть товаров не найдена по external_id', [
                    'uuid' => $uuid,
                    'missing' => $missing,
                ]);
            }
            ...
            Log::info('promotion.created: обработано', [
                'uuid' => $uuid,
                'type' => $type,
