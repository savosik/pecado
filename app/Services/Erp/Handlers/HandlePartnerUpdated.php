 ...
 *
 * `name` из payload приезжает в `users.erp_name` — рабочее наименование карточки
 * партнёра. Личное `users.name` принадлежит клиенту и из 1С не перезаписывается.
 *
 * Два сценария поиска:
...
class HandlePartnerUpdated
{
    ...
    private function updateUser(User $user, array $payload, bool $bindErpId = false, int|null|false $personalManagerId = false): void
    {
        ...
        // name → рабочее наименование. Личное `name` не трогаем: клиент правит его
        // в кабинете, а раньше следующее сообщение из 1С молча возвращало старое
        // значение — менеджеры и клиент видели разные имена одного человека.
        if (isset($payload['name'])) {
            $updateData['erp_name'] = $payload['name'];
        }
