 ...
 * есть история, у остальных — «оценить нельзя». Раздел открывается без фильтров;
 * чипы «с историей покупок» и «ушедшие» включаются явно.
 *
...
class PoolListService
{
    ...
    public function list(int $managerId, CarbonInterface $month, array $query = []): array
    {
        ...
        // чипы «с историей» и «ушедшие» включаются явно.
        $historyOnly = in_array($query['history'] ?? 0, [1, '1', true, 'true'], true);
