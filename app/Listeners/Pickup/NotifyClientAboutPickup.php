        ...
        foreach ($byClient as $pairs) {
            ...
            $this->mailStream->captureQuietly(new Occasion(
                ...
                view: [
                    ...
                    'body' => sprintf(
                        '%s: собрано%s. Выдаём %s по адресу: %s. Выпустите в кабинете пропуск и перешлите его курьеру — на складе он покажет QR-код, подписывать ничего не нужно.',
                        $label,
                        $packages > 0 ? ", мест: {$packages}" : '',
                        $until,
